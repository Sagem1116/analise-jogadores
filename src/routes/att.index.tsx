...
import { setAttPlayers, type PlayerRow } from "@/lib/att-store";
import { toast } from "sonner";

export const Route = createFileRoute("/att/")({
  head: () => ({ meta: [{ title: "Att Upload | FMDataLab" }] }),
  component: AttUpload,
});
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-[1200px] px-6 py-10">
        <div className="mb-6 flex items-end justify-between">
          <div>
            <h1 className="text-3xl font-bold">Att <span className="ml-2 rounded bg-success/20 px-2 py-1 text-xs font-bold text-success">UPDATE</span></h1>
            <p className="mt-1 text-muted-foreground">Faça upload do seu ficheiro de atributos (.html ou .csv)</p>
          </div>
          <Link to="/att/weights" className="rounded-md border border-border bg-card px-4 py-2 text-sm hover:border-primary">
            Att Score Weight
          </Link>
        </div>
