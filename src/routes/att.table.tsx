...
import { useAttPlayers, computePercentiles, computeNormalized, isNumericColumn } from "@/lib/att-store";
...
// Columns that should NEVER show as percentile cards / never be used as attribute weights
...
function AttTable() {
  ...
  const normalizedMap = useMemo(() => computeNormalized(players, [...numericCols]), [players, numericCols]);
  ...
  const roleScores = useMemo(() => {
    ...
    for (let idx = 0; idx < players.length; idx++) {
      ...
      for (const role of selectedRoles) {
        ...
        for (const stat in w) {
          const info = w[stat];
          const pctMap = percentilesMap.get(stat);
          const normMap = normalizedMap.get(stat);
  ...
  const filtered = useMemo(() => {
    ...
    return out;
  }, [players, search, colFilters, sortBy, selectedRoles, roleScores, nameCol]);
  ...
  function exportExcel() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Att");
    XLSX.writeFile(wb, "fmdatalab-att.xlsx");
  }
    ...
    return (
      <div className="min-h-screen">
        ...
        <main className="mx-auto max-w-[1200px] px-6 py-20 text-center">
          ...
          <Link to="/att" className="mt-4 inline-block rounded bg-primary px-4 py-2 text-sm font-medium">Ir para upload</Link>
        </main>
      </div>
    );
  }
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-[1800px] px-4 py-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold">{numericCols.size} atributos</h1>
            <p className="text-sm text-muted-foreground">{players.length.toLocaleString()} Total Players</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link to="/att" className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-semibold btn-glow">
              <Download className="h-4 w-4 rotate-180" /> Upload
            </Link>
            <Link to="/att/weights" className="inline-flex items-center gap-2 rounded-md border border-border bg-card px-4 py-2 text-sm hover:border-primary">
              <Settings2 className="h-4 w-4" /> Att Score Weight
            </Link>
